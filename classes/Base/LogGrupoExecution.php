...
/**
 * Base class that represents a row from the 'log_grupo_execution' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class LogGrupoExecution implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\LogGrupoExecutionTableMap';
    ...
    /**
     * Initializes internal state of Base\LogGrupoExecution object.
     */
    ...
    /**
     * Compares this with another <code>LogGrupoExecution</code> instance.  If
     * <code>obj</code> is an instance of <code>LogGrupoExecution</code>, delegates to
     * <code>equals(LogGrupoExecution)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     * @return $this|\LogGrupoExecution The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[LogGrupoExecutionTableMap::COL_ID] = true;
        }
    ...
    public function setGrupoScheduleId($v)
    {
        ...
        if ($this->grupo_schedule_id !== $v) {
            ...
            $this->modifiedColumns[LogGrupoExecutionTableMap::COL_GRUPO_SCHEDULE_ID] = true;
        }
    ...
    public function setPushPop($v)
    {
        ...
        if ($this->push_pop !== $v) {
            $this->push_pop = $v;
            $this->modifiedColumns[LogGrupoExecutionTableMap::COL_PUSH_POP] = true;
        }
    ...
    public function setLogtime($v)
    {
        ...
        if ($this->logtime !== $v) {
            $this->logtime = $v;
            $this->modifiedColumns[LogGrupoExecutionTableMap::COL_LOGTIME] = true;
        }
    ...
    public function setDetails($v)
    {
        ...
        if ($this->details !== $v) {
            $this->details = $v;
            $this->modifiedColumns[LogGrupoExecutionTableMap::COL_DETAILS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : LogGrupoExecutionTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : LogGrupoExecutionTableMap::translateFieldName('GrupoScheduleId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->grupo_schedule_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : LogGrupoExecutionTableMap::translateFieldName('PushPop', TableMap::TYPE_PHPNAME, $indexType)];
            $this->push_pop = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : LogGrupoExecutionTableMap::translateFieldName('Logtime', TableMap::TYPE_PHPNAME, $indexType)];
            $this->logtime = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : LogGrupoExecutionTableMap::translateFieldName('Details', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = LogGrupoExecutionTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\LogGrupoExecution'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildLogGrupoExecutionQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see LogGrupoExecution::setDeleted()
     * @see LogGrupoExecution::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildLogGrupoExecutionQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                LogGrupoExecutionTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('log_grupo_execution_id_seq')");
        ...
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_GRUPO_SCHEDULE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'grupo_schedule_id';
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_PUSH_POP)) {
            $modifiedColumns[':p' . $index++]  = 'push_pop';
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_LOGTIME)) {
            $modifiedColumns[':p' . $index++]  = 'logtime';
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_DETAILS)) {
            $modifiedColumns[':p' . $index++]  = 'details';
        }

        $sql = sprintf(
            'INSERT INTO log_grupo_execution (%s) VALUES (%s)',
    ...
    {
        $pos = LogGrupoExecutionTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['LogGrupoExecution'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['LogGrupoExecution'][$this->hashCode()] = true;
        $keys = LogGrupoExecutionTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\LogGrupoExecution The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_ID)) {
            $criteria->add(LogGrupoExecutionTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_GRUPO_SCHEDULE_ID)) {
            $criteria->add(LogGrupoExecutionTableMap::COL_GRUPO_SCHEDULE_ID, $this->grupo_schedule_id);
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_PUSH_POP)) {
            $criteria->add(LogGrupoExecutionTableMap::COL_PUSH_POP, $this->push_pop);
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_LOGTIME)) {
            $criteria->add(LogGrupoExecutionTableMap::COL_LOGTIME, $this->logtime);
        }
        if ($this->isColumnModified(LogGrupoExecutionTableMap::COL_DETAILS)) {
            $criteria->add(LogGrupoExecutionTableMap::COL_DETAILS, $this->details);
        }
    ...
    {
        $criteria = ChildLogGrupoExecutionQuery::create();
        $criteria->add(LogGrupoExecutionTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \LogGrupoExecution (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \LogGrupoExecution Clone of current object.
     * @throws PropelException
     */
    ...
    public function getGrupoSchedule(ConnectionInterface $con = null)
    {
        if ($this->aGrupoSchedule === null && ($this->grupo_schedule_id != 0)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aGrupoSchedule->addLogGrupoExecutions($this);
             */
        }
    ...
    {
        if (null !== $this->aGrupoSchedule) {
            $this->aGrupoSchedule->removeLogGrupoExecution($this);
        }
        $this->id = null;
    ...
    {
        return (string) $this->exportTo(LogGrupoExecutionTableMap::DEFAULT_STRING_FORMAT);
    }
